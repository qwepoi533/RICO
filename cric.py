...
class CRIC(object):
    ...
    Attributes:
        ...
        decay: float, the decay rate for the history records, only for online style.
    ...
    def __init__(self, n_freq, n_conf, max_depth=1000, max_size=4, n_chain=300,
        ...
        self.online = online
        self.decay = decay if online else 1
        self.t = 0
        self.num_cls = {}
        self.freqs = None
    ...
    def select_frequent_set(self, chains):
        ...
        for c in chains:
            for item, count in chains[c]:
                ...
                # print(max_freq_item)
                if depth and 0 < len(max_freq_item) <= self.max_size:
...
def load_cric(path):
    ...
    if os.path.exists(path):
        ...
        for attr in attr_dict:
            cric.__dict__[attr] = attr_dict[attr]
        return cric
    else:
